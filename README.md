...
### Step 1: Running the Backend API Server

First, we need to start our Python server. It will wait for requests from the browser extension.

1.  **Open Your Terminal:** Open a new terminal or command prompt window.

2.  **Navigate to the Backend Directory:** Use the `cd` command to move into the `backend` folder of our project.
    ```bash
    cd path/to/your/project/social-compatibility-analyzer/backend
    ```

3.  **Start the Server:** Run the following command. This tells `uvicorn` to run the `app` object inside the `main.py` file. The `--reload` flag makes the server restart automatically if you make any code changes.
    ```bash
    uvicorn main:app --reload
    ```

4.  **Check the Output:** If successful, you will see output in your terminal indicating that the server is running, typically on `http://127.0.0.1:8000`. It should look something like this:
    ```
    ...
    ```
**Leave this terminal window running.** If you close it, the backend server will stop.

---

### Step 2: Loading the Chrome Extension

Now, we'll load our extension into Chrome.

1.  **Open Chrome Extensions Page:** Open Google Chrome, type `chrome://extensions` into the address bar, and press Enter.

2.  **Enable Developer Mode:** In the top-right corner of the Extensions page, find the "Developer mode" toggle and make sure it is switched **ON**.

3.  **Load the Extension:** You should now see three new buttons: "Load unpacked," "Pack extension," and "Update." Click on the **"Load unpacked"** button.

4.  **Select the Folder:** A file dialog will open. Navigate to your project directory and select the **`extension`** folder. **Do not select any files inside it; select the entire `extension` folder itself** and click "Select Folder".

5.  **Confirm:** If successful, you will see a new card for "Social Compatibility Analyzer" appear on your Extensions page.

---

### Step 3: Putting It All Together

With the backend running and the extension loaded, you're ready to test!

1.  **Open Twitter:** Navigate to `https://twitter.com` in a new Chrome tab.

2.  **Find a Profile:** Scroll through your timeline or go to any user's profile page.

3.  **Hover!** Move your mouse over any link that leads to a user's profile (their name or profile picture). After a brief delay (700ms), our hover-card should appear, first showing a loading spinner and then updating with the compatibility score from your backend.

If you see the card with a score and shared interests, congratulations! The MVP is fully working.
#   C o m p a t i b i l i t y - R e p o r t  
 